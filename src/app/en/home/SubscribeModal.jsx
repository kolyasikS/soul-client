...
const CreateClubModal = ({close}) => {
    ...
    const apply = async (e) => {
        ...
        if (ctx) {
            ...
            if (!res.error) {
                localStorage.setItem('soul-user', JSON.stringify({...parsedCtx, subscription: res}));
                close();
    ...
    return (
        <ClassicDialog zIndex={9999999}>
            <form className={styles.form}>
                <p className={'text-lg font-bold text-blue-300'}>Current price of 1 season - <span className={'text-red-600'}>{price}</span></p>
                ...
                <p className={'text-lg font-bold text-blue-300'}>Total cost - <span className={'text-red-600'}>{+(price.slice(0, price.length - 1)) * (isNaN(seasons) ? 0 : seasons)} $</span></p>
